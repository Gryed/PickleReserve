...
export default function Home() {
  ...
  if (loading) return <div className="p-8 max-w-4xl mx-auto text-muted">Loading courts...</div>
  if (error) return <div className="p-8 max-w-4xl mx-auto text-red-400">{error}</div>

  return (
    <div className="p-6 sm:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="font-display text-3xl font-semibold text-ink mb-1">Book a court</h1>
        <p className="text-muted">Pick a court and reserve your time slot.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {courts.map((court) => (
          <Link
            key={court.id}
            to={`/booking/${court.id}`}
            className="group border border-line rounded-lg p-5 bg-surface hover:border-court transition-colors"
          >
            <div className="flex items-start justify-between mb-2">
              <p className="font-display font-semibold text-lg text-ink">{court.name}</p>
              <span className="text-xs px-2 py-1 rounded-full bg-court/15 text-court font-medium">
                Available
              </span>
            </div>
            {settings?.show_court_type && court.type && (
              <p className="text-sm text-muted mb-2">{court.type}</p>
            )}
